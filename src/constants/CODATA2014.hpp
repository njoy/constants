...
struct PlanckLength : hana::type< PlanckLength >{
  ...
  constexpr decltype( auto ) operator()( MAP map ) const {
    auto tmp = map[ hbar ]*map[ G ]/( map[ c ]*map[ c ]*map[ c ] );
    return dimwits::math::Pow::evaluate( tmp.value, Ratio<1,2> )*
        sqrt( tmp.units() );
  }
};
static constexpr PlanckLength planckLength{};
...
constexpr auto derivedConstants = hana::make_tuple( planckLength );

constexpr auto CODATA2014 = hana::fold( derivedConstants,
  hana::make_map(
...
// auto CODATA2014_ = hana::fold( tuple, CODATA2014, insert );

constexpr auto CODATA2014Uncertainty = hana::make_map(
